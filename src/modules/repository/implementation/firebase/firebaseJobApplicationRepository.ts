  ...
  async getApplications(pseudo: string): Promise<Array<JobApplication> | undefined> {
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        data.societyName = data?.companyInformation?.societyName ?? data.societyName;
        data.societyWebSite = data?.companyInformation?.societyWebSite ?? data.societyWebsite;
        ...
        data.responseDate = data.responseDate
          ...
          : undefined;
        result.push(JobApplication.createJobApplication(data));
      });
