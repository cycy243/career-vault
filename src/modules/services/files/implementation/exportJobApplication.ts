...
import { utils, write as writeExcel } from 'xlsx-js-style';
...
export default class ExportJobApplication implements IExportJobApplication {
  ...
  exportToExcel(jobApplications: Array<JobApplication>): Promise<boolean> {
    try {
      ...
      utils.book_append_sheet(workBook, workSheet, 'job tracking');
      // Exporting the file with the desired name and extension.
      const excelBuffer = writeExcel(workBook, { bookType: 'xlsx', type: 'array' });
      const fileData = new Blob([excelBuffer], { type: fileType });
      saveAs(fileData, this.generateFileName() + excelFileExtension);
      return Promise.resolve(true);
    } catch (error) {
      throw new ServiceError('An error occured while exporting the PDF');
    }
  ...
  convertToArray(application: JobApplication): Array<string> {
    ...
    result.push(application.companyInformation.societyName);
    result.push(application.jobTitle ?? 'Spontaneous application');
